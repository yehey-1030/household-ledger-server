...
public interface TagLedgerRelationRepository  extends JpaRepository<TagLedgerRelation,Long> {
    ...
    @Query(value = "select r.ledger.ledgerID from TagLedgerRelation r where r.tag.tagID=:tagID and r.ledger.date>:start and r.ledger.date<:end and r.ledger.archiveTypeID.archiveTypeID=:archiveTypeID")
